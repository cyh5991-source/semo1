...
const P = {
  ...
  red: "#FF4757",
  redSoft: "rgba(255,71,87,0.06)",
  redMid: "rgba(255,71,87,0.25)",
  orange: "#FF8C42",
...
const KAKAO_PIXEL_ID = "5081631125761573189";
...
function calcResult(a: Record<string, any>) {
  ...
  const total36 = mp * 36;
  const reduction = debt > 0 ? Math.max(Math.min(Math.round((1 - total36 / debt) * 100), 95), 0) : 0;
...
/* ── 동의 상세 모달 ── */
function ConsentModal({ type, onClose }: { type: "required" | "optional"; onClose: () => void }) {
  return (
    <div onClick={onClose}
      ...
      <div onClick={e => e.stopPropagation()}
        style={{ background: "#fff", borderRadius: 16, padding: 24, maxWidth: 400, width: "100%", maxHeight: "80vh", overflow: "auto" }}>
        {type === "required" ? (
          <>
            <h3 style={{ fontSize: 16, fontWeight: 800, color: P.navy, marginBottom: 12 }}>[필수] 개인정보 수집·이용 동의</h3>
            <div style={{ fontSize: 13, color: P.textMid, lineHeight: 1.8 }}>
              <p style={{ marginBottom: 10 }}><strong style={{ color: P.navy }}>1. 수집 항목</strong><br />휴대폰번호, 설문 응답 내용</p>
              <p style={{ marginBottom: 10 }}><strong style={{ color: P.navy }}>2. 수집·이용 목적</strong><br />본인인증, 상담 서비스 제공, 상담 연락</p>
              <p style={{ marginBottom: 10 }}><strong style={{ color: P.navy }}>3. 보유·이용 기간</strong><br />수집일로부터 1년 (상담 완료 후 즉시 파기)</p>
              <p style={{ marginBottom: 10 }}><strong style={{ color: P.navy }}>4. 동의 거부 권리</strong><br />본 동의를 거부할 권리가 있으며, 거부 시 서비스 이용이 제한될 수 있습니다.</p>
            </div>
          </>
        ) : (
          <>
            <h3 style={{ fontSize: 16, fontWeight: 800, color: P.navy, marginBottom: 12 }}>[선택] 마케팅 정보 수신 동의</h3>
            <div style={{ fontSize: 13, color: P.textMid, lineHeight: 1.8 }}>
              <p style={{ marginBottom: 10 }}><strong style={{ color: P.navy }}>1. 수신 내용</strong><br />법률 서비스 안내, 이벤트 정보, 유용한 법률 정보</p>
              <p style={{ marginBottom: 10 }}><strong style={{ color: P.navy }}>2. 수신 방법</strong><br />카카오톡, 문자메시지(SMS)</p>
              <p style={{ marginBottom: 10 }}><strong style={{ color: P.navy }}>3. 동의 거부 권리</strong><br />본 동의는 선택사항이며, 거부하셔도 상담 서비스 이용에 제한이 없습니다.</p>
            </div>
          </>
        )}
        <button onClick={onClose}
          style={{ width: "100%", padding: 12, marginTop: 16, borderRadius: 10, background: P.accent, color: "#fff", fontSize: 14, fontWeight: 700, border: "none", cursor: "pointer" }}>
          확인
        </button>
      </div>
    </div>
...
function VerifyStep({ onNext, onBack, answers }: any) {
  ...
  const [modalType, setModalType] = useState<"required" | "optional" | null>(null);
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const intervalRef = useRef<NodeJS.Timeout | null>(null);
  const result = calcResult(answers);

  const allAgreed = agreedRequired && agreedOptional;

  const toggleAll = () => {
    const next = !allAgreed;
    setAgreedRequired(next);
    setAgreedOptional(next);
    setError("");
  };
  ...
  const sendCode = async () => {
    if (!agreedRequired) { setError("필수 개인정보 수집·이용에 동의해 주세요."); return; }
  ...
  const verifyCode = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        try {
          const submitRes = await fetch("/api/submit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ verificationId, phone, answers, result, marketingAgreed: agreedOptional }) });
          ...
          {!sent ? (
            <>
              ...
              <div onClick={toggleAll}
                style={{ border: `1.5px solid ${P.accent}`, borderRadius: 10, padding: "11px 14px", background: "rgba(59,106,255,0.04)", marginBottom: 8, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 20, height: 20, borderRadius: 6, flexShrink: 0, border: `2px solid ${P.accent}`, background: allAgreed ? P.accent : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {allAgreed && <span style={{ color: "#fff", fontSize: 12, fontWeight: 700 }}>✓</span>}
                </div>
                <p style={{ fontSize: 13, fontWeight: 700, color: P.accent, margin: 0 }}>전체 동의하기</p>
              </div>

              {/* 개별 항목 */}
              <div style={{ background: P.bg, borderRadius: 10, padding: "10px 14px", marginBottom: 16 }}>
                {/* 필수 */}
                <div style={{ display: "flex", alignItems: "center", gap: 10, paddingBottom: 8, borderBottom: `1px solid ${P.border}`, marginBottom: 8 }}>
                  <div onClick={() => { setAgreedRequired(!agreedRequired); setError(""); }}
                    style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0, border: `1.5px solid ${P.accent}`, background: agreedRequired ? P.accent : "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                    {agreedRequired && <span style={{ color: "#fff", fontSize: 10, fontWeight: 700 }}>✓</span>}
                  </div>
                  <p onClick={() => { setAgreedRequired(!agreedRequired); setError(""); }} style={{ fontSize: 12, color: P.navy, fontWeight: 500, flex: 1, cursor: "pointer", margin: 0 }}>
                    <span style={{ color: P.red, fontWeight: 700 }}>[필수]</span> 개인정보 수집·이용 동의
                  </p>
                  <a onClick={() => setModalType("required")} style={{ fontSize: 10, color: P.textLight, textDecoration: "underline", cursor: "pointer" }}>보기</a>
                </div>
                {/* 선택 */}
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div onClick={() => setAgreedOptional(!agreedOptional)}
                    style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0, border: `1.5px solid ${P.accent}`, background: agreedOptional ? P.accent : "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                    {agreedOptional && <span style={{ color: "#fff", fontSize: 10, fontWeight: 700 }}>✓</span>}
                  </div>
                  <p onClick={() => setAgreedOptional(!agreedOptional)} style={{ fontSize: 12, color: P.navy, fontWeight: 500, flex: 1, cursor: "pointer", margin: 0 }}>
                    <span style={{ color: P.textLight, fontWeight: 700 }}>[선택]</span> 마케팅 정보 수신 동의
                  </p>
                  <a onClick={() => setModalType("optional")} style={{ fontSize: 10, color: P.textLight, textDecoration: "underline", cursor: "pointer" }}>보기</a>
                </div>
              </div>
...
function ResultPage({ answers, onRestart }: any) {
  ...
  /* 가용소득 없는 경우 → 새로운 소득부족 화면 */
  if (r.mp === 0) {
    return (
      <div style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", padding: "28px 16px" }}>
        <div style={{ maxWidth: 440, width: "100%" }}>

          {/* 배지 */}
          <div style={{ textAlign: "center", marginBottom: 18 }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 16px", borderRadius: 24, background: P.redSoft, color: P.red, fontSize: 12, fontWeight: 700 }}>
              <span style={{ width: 7, height: 7, borderRadius: "50%", background: P.red, display: "inline-block", animation: "pulse 1.2s infinite" }} />
              즉시 조치가 필요합니다
            </span>
          </div>
          ...
          <div style={{ textAlign: "center", marginBottom: 20 }}>
            <h2 style={{ fontFamily: "'Outfit','Noto Sans KR'", fontSize: 26, fontWeight: 900, letterSpacing: -1, color: P.navy, lineHeight: 1.3, marginBottom: 8, marginTop: 0 }}>
              회생이 어렵다고<br /><span style={{ color: P.accent }}>끝이 아닙니다</span>
            </h2>
            <p style={{ fontSize: 13, color: P.textMid, lineHeight: 1.7, margin: 0 }}>
              소득이 낮아도 채무 부담을 크게 줄이거나<br />완전히 해결할 수 있는 방법이 있습니다
            </p>
          </div>

          {/* 변호사 */}
          <div style={{ display: "flex", alignItems: "center", gap: 14, background: P.surface, borderRadius: 16, padding: "16px 18px", border: `1px solid ${P.border}`, marginBottom: 16 }}>
            <img src={LAWYER_IMG2} alt="이우형 변호사" style={{ width: 56, height: 56, borderRadius: "50%", objectFit: "cover", border: `3px solid ${P.accent}`, flexShrink: 0 }} />
            <div>
              <p style={{ fontSize: 15, fontWeight: 700, color: P.navy, margin: "0 0 2px" }}>이우형 변호사의 분석 결과</p>
              <p style={{ fontSize: 12, color: P.textMid, margin: 0 }}>더엘 법률사무소 · 개인회생 6,000건+</p>
            </div>
          </div>

          {/* 위험 경고 박스 */}
          <div style={{ background: P.redSoft, border: `1.5px solid ${P.redMid}`, borderRadius: 16, padding: "18px 20px", marginBottom: 16 }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: P.red, margin: "0 0 10px" }}>⚠️ 지금 방치하면 이렇게 됩니다</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: P.red, flexShrink: 0 }} />
                <p style={{ fontSize: 13, color: P.navy, fontWeight: 500, margin: 0 }}><strong style={{ color: P.red }}>3개월 내</strong> 급여·재산 압류 시작</p>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: P.red, flexShrink: 0 }} />
                <p style={{ fontSize: 13, color: P.navy, fontWeight: 500, margin: 0 }}>이자·연체료 누적으로 <strong style={{ color: P.red }}>채무 급증</strong></p>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: P.red, flexShrink: 0 }} />
                <p style={{ fontSize: 13, color: P.navy, fontWeight: 500, margin: 0 }}>신용불량 장기화로 <strong style={{ color: P.red }}>금융거래 전면 제한</strong></p>
              </div>
            </div>
          </div>

          {/* 희망 섹션 */}
          <div style={{ background: `linear-gradient(135deg, ${P.navy}, ${P.navyLight})`, borderRadius: 16, padding: 20, marginBottom: 16 }}>
            <p style={{ fontSize: 14, fontWeight: 700, color: "#fff", margin: "0 0 8px" }}>💡 개인회생이 전부가 아닙니다</p>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.75)", lineHeight: 1.75, margin: 0 }}>
              소득이 낮거나 없는 경우에도 활용할 수 있는 채무조정 방법이 있습니다. 어떤 방법이 본인에게 맞는지는 상황에 따라 다르기 때문에, 전문 변호사와 함께 정확히 확인하는 것이 중요합니다.
            </p>
          </div>

          {/* 채무조정 방법 카드 2개 */}
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
            <div style={{ background: P.surface, borderRadius: 16, padding: "18px 20px", border: `2px solid ${P.accent}`, position: "relative" }}>
              <span style={{ position: "absolute", top: -11, left: 16, background: P.accent, color: "#fff", fontSize: 11, fontWeight: 700, padding: "3px 12px", borderRadius: 20 }}>이우형 변호사 추천</span>
              <p style={{ fontSize: 14, fontWeight: 700, color: P.accent, margin: "0 0 6px" }}>⚖️ 개인파산·면책</p>
              <p style={{ fontSize: 13, color: P.textMid, lineHeight: 1.7, margin: 0 }}>
                변제 능력이 없는 경우 법원을 통해 채무를 <strong>전액 면책</strong> 받을 수 있습니다. 신청 후 평균 <strong style={{ color: P.accent }}>6~8개월</strong> 내 면책 결정이 납니다.
              </p>
            </div>

            <div style={{ background: P.surface, borderRadius: 16, padding: "18px 20px", border: `1px solid ${P.border}` }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: P.textMid, margin: "0 0 6px" }}>📋 기타 채무조정 제도</p>
              <p style={{ fontSize: 13, color: P.textMid, lineHeight: 1.7, margin: 0 }}>
                본인 상황에 따라 파산 외에도 더 유리한 방법이 있을 수 있습니다. <strong style={{ color: P.navy }}>어떤 방법이 맞는지는 상담을 통해서만 정확히 확인할 수 있습니다.</strong>
              </p>
              <div style={{ marginTop: 10, padding: "10px 14px", background: P.bg, borderRadius: 10, display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 12 }}>🔍</span>
                <p style={{ fontSize: 12, color: P.accent, fontWeight: 600, margin: 0 }}>이우형 변호사가 무료로 확인해 드립니다</p>
              </div>
            </div>
          </div>
          ...
          <button onClick={() => alert("상담 신청이 접수되었습니다. 이우형 변호사팀에서 곧 연락드립니다.")}
            style={{ width: "100%", padding: "19px 32px", borderRadius: 16, background: `linear-gradient(135deg, ${P.accent}, #7C5CFC)`, color: "#fff", fontSize: 16, fontWeight: 700, border: "none", cursor: "pointer", boxShadow: "0 4px 24px rgba(59,106,255,0.3)", marginBottom: 12 }}>
            📞 이우형 변호사에게 내 방법 찾기 →
          </button>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    kakaoScript.src = "https://t1.daumcdn.net/kas/static/kp.js";
    kakaoScript.async = true;
    kakaoScript.onload = () => {
      try {
        (window as any).kakaoPixel(KAKAO_PIXEL_ID).pageView();
      } catch (err) {
        console.error("[Kakao Pixel] pageView 오류", err);
      }
    };
  ...
  useEffect(() => {
    if (phase === "result") {
      ...
      if (typeof (window as any).fbq !== "undefined") {
        try { (window as any).fbq("track", "Lead"); } catch (err) { console.error("[Meta Pixel] Lead 오류", err); }
      }
      // Kakao 가입완료 이벤트
      if (typeof (window as any).kakaoPixel !== "undefined") {
        try { (window as any).kakaoPixel(KAKAO_PIXEL_ID).completeRegistration(); } catch (err) { console.error("[Kakao Pixel] completeRegistration 오류", err); }
      }
    }
  }, [phase]);
